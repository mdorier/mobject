...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if(g_operation == "create") {
            auto id = admin.createSequencer(g_address, g_provider_id,
                g_type, g_config, g_token);
            spdlog::info("Created sequencer {}", id.to_string());
        } else if(g_operation == "open") {
            auto id = admin.openSequencer(g_address, g_provider_id,
                g_type, g_config, g_token);
            spdlog::info("Opened sequencer {}", id.to_string());
        } else if(g_operation == "close") {
            admin.closeSequencer(g_address, g_provider_id,
                mobject::UUID::from_string(g_sequencer.c_str()), g_token);
            spdlog::info("Closed sequencer {}", g_sequencer);
        } else if(g_operation == "destroy") {
            admin.destroySequencer(g_address, g_provider_id,
                mobject::UUID::from_string(g_sequencer.c_str()), g_token);
            spdlog::info("Destroyed sequencer {}", g_sequencer);
        }

        // Any of the above functions may throw a mobject::Exception
...
void parse_command_line(int argc, char** argv) {
    try {
        TCLAP::CmdLine cmd("Mobject admin", ' ', "0.1");
        ...
        TCLAP::ValueArg<std::string> typeArg("t","type","Sequencer type", false,"dummy","string");
        TCLAP::ValueArg<std::string> sequencerArg("r","sequencer","Sequencer id", false, mobject::UUID().to_string(),"string");
        TCLAP::ValueArg<std::string> configArg("c","config","Sequencer configuration", false,"","string");
